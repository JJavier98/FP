...
public:
	...
	SecuenciaCaracteres CountingSort(){
		char mayor, menor;
		int repeticiones[utilizados_caracteres];
		
		mayor = v_caracteres[0]
		menor = v_caracteres[0];
		
		for(int i = 1 ; i < utilizados_caracteres ; i++){
			if (mayor < v_caracteres[i]){
				mayor = v_caracteres[i];
			}
			if (menor > v_caracteres[i]){
				menor = v_caracteres[i];
			}
		}
	}
