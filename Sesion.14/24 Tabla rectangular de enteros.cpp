...
private:
	static const MAX_FILAS_COLUMNAS = 500;
	int matriz_privada[MAX_FILAS_COLUMNAS][MAX_FILAS_COLUMNAS];
	const int numero_filas = 0,
				 numero_columnas = 0;
...
public:
	TablaRectangularEnteros(int filas, int columnas)
		:numero_filas(filas), numero_columnas(columnas)
		{
		}
	...
	int Valor(int fila, int columna){
		int dato;
		dato = matriz_privada[fila][columna];
		return dato;
	}

	SecuenciaEnteros DevuelveFila(int indice){
		SecuenciaEnteros fila;

		
		for (int col = 0 ; col < numero_columnas ; col++){
			fila.vector_privado[total_utilizados] = matriz_privada[indice][col];
		}
		return fila;
	}
	...
	void AniadeFila(SecuenciaReales nueva_fila){
	   if (indice_ultima_fila_aniadida < MAX_FILAS_COLUMNAS){
	      if (nueva_fila.TotalUtilizados() == numero_columnas){
   ...
   bool EsIgual(TablaRectangularEnteros tabla){
   	bool es_igual = numero_filas == tabla.Fila() && numero_columnas == tabla.Columna();
   	
		for(int i = 0 ; i < numero_filas && es_igual ; i++){
			for ( int j = 0 ; j < numero_columnas && es_igual ; j++){
				es_igual = tabla.Valor(i,j) == matriz_privada[i][j])
			}
		}
   	return es_igual;
   }
   ...
   TablaRectangularEnteros Traspuesta(){
   	TablaRectangularEnteros traspuesta(numero_columnas, numero_filas);
   ...
   bool EsSimetrica(TablaRectangularEnteros simetrica){
